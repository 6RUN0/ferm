...
B<ferm> rollback [--list [-n I<N>] | --diff [I<SHA>] | --to I<SHA>] [I<config>]
...
=item B<-n, --noexec>
Do not execute the netfilter commands, just simulate.  Combine with B<--lines> to inspect the generated rules.

=item B<-F, --flush>
Clear the firewall rules and set the policy of every affected chain to ACCEPT.  B<ferm> still needs the configuration file to determine which domains and tables are affected.

=item B<--noflush>
Apply without flushing first: rules in undeclared chains are kept, declared chains are overwritten and policies applied.  Re-running appends the declared rules again -- see the caveats B<--plan> prints for noflush diffs.

=item B<-l, --lines>
Show the firewall lines generated from the rules, just before they are executed -- an iptables(8) error can then be matched to the rule that caused it.

=item B<-i, --interactive>
Apply the rules and ask for confirmation; revert to the previous ruleset when no valid response arrives within the timeout.  The safety net for remote administration.

=item B<-t, --timeout> I<s>
Wait this many seconds for the B<--interactive> confirmation before reverting (default 30).
...
=item B<--test, --remote>
Test mode: parse and build everything while ignoring host-specific state (kernel readback, tool discovery).  Implies B<--noexec> and B<--lines>.  B<--remote> is an alias kept for compatibility.

=item B<--test-mock-previous> I<fam=path>
Simulation: substitute a saved dump for the named family's previous kernel state.  Enables offline what-if planning without root: C<ferm --test --test-mock-previous ip=snap.save --plan ferm.conf>.  May repeat, one family per occurrence.

=item B<--fast>
Use iptables-restore(8) for an atomic install.  This is the default; the switch exists for symmetry with B<--slow>.

=item B<--slow>
Run one iptables(8) process per rule instead of an atomic iptables-restore(8) install.

=item B<--shell>
Print a shell script that feeds the generated rules to iptables-restore(8); implies B<--lines>.

=item B<--domain> I<{ip|ip6}>
Restrict processing to one domain.  The configuration is still parsed as a whole; other domains are skipped.

=item B<--def> I<'$name=v'>
Override a configuration variable from the command line; may repeat.

=item B<--nolegacy>
Skip the C<*-legacy> tool family during tool discovery -- for hosts where the legacy binaries exist but the nf_tables-based ones own the kernel state.

=item B<--nft>
Install the ruleset natively through nft(8) instead of iptables-restore(8).  See the NFT BACKEND section.

=item B<--full-reload>
Opt out of the incremental delta apply under B<--nft>: flush the ferm-owned tables and reload from scratch (counters reset).

=item B<--no-etckeeper>
Do not record the applied ruleset in the etckeeper(8) history.  See the ETCKEEPER INTEGRATION section.

=item B<--plan>
Compute the ruleset and report what would change against the running kernel, applying nothing.  Exit status 2 signals pending changes.  See the PLAN MODE section.

=item B<--plan-format> I<F>
Select the B<--plan> renderer: C<structured> (default, per-family change list) or C<diff> (a unified diff).

=item B<--lint>
Eval-free static analysis of the configuration: report findings and apply nothing.  See the LINT MODE section.

=item B<--lint-strict>
Gate B<--lint> at warning level: any finding at warning or above yields exit status 2.

=item B<--lint-fail-level> I<L>
Gate B<--lint> at an explicit severity: findings at or above I<L> yield exit status 2.

=item B<--list-modules>
Print every supported protocol/match/target module and keyword, then exit.  See the INTROSPECTION section.

=item B<--describe> I<NAME>
Describe one name -- a module, an option, a builtin, a shortcut or a deprecated keyword; unknown names get close-match suggestions.

=item B<--graph>
Print the configuration's chain control-flow graph without evaluating or applying it.

=item B<--graph-format> I<F>
Select the B<--graph> renderer: C<d2> (default) or C<dot>.

=back

=head1 NFT BACKEND

With B<--nft> the ruleset is installed natively through nft(8)
instead of iptables-restore(8): everything lands in one C<table ferm>
per address family (nat/mangle chains carry a C<nat_>/C<mangle_>
prefix), the desired ruleset is pre-checked with C<nft -c> before
anything touches the kernel, and repeat runs apply an incremental
delta (see B<--full-reload> to opt out).  Constructs whose nft
translation is not implemented are refused loudly rather than
translated approximately.

=head1 PLAN MODE

B<--plan> computes the ruleset and reports what would change against
the running kernel without applying anything, for both backends.
Exit status 0 means no changes, 2 means changes are pending, 1 an
error.  B<--plan-format diff> renders a unified diff instead of the
structured per-family listing.  Combined with B<--test> and
B<--test-mock-previous> it answers what-if questions offline,
without root.

=head1 LINT MODE

B<--lint> runs eval-free static analysis over the configuration:
unreachable chains, shadowed variables and functions, deprecated
keywords and jump cycles are reported without touching the kernel.
B<--lint-strict> (or an explicit B<--lint-fail-level>) turns
findings into exit status 2 for CI gating.

=head1 INTROSPECTION

B<--list-modules> prints every supported protocol, match and target
module with its keywords; B<--describe> I<NAME> shows one module,
option, builtin function, shortcut or deprecated keyword.  Unknown
names get close-match suggestions (C<did you mean>).  B<--graph>
prints the chain control-flow graph of a configuration in d2 or DOT
format for visualisation.

=head1 ROLLBACK

B<ferm rollback> reverts the configuration directory to a recorded
revision and re-applies it, so the kernel always matches the
reverted files; it requires /etc under etckeeper(8) managed by git.
B<--list> shows the dated revision history (first line marked
C<(current)>; B<-n> caps it), B<--diff> previews a revision delta
read-only, the bare form reverts one ferm revision back after
showing the diff and asking for confirmation, and B<--to> I<SHA> is
the scriptable exact-revision form.  Backend and mode switches
(B<--nft>, B<--slow>, B<--nolegacy>, B<--def>, ...) are inherited by
the re-apply so an nft install rolls back under nft.

=head1 ETCKEEPER INTEGRATION

After a successful apply that touched the kernel, B<ferm> records
the change as an etckeeper(8) commit whose subject carries the verb
and the counted delta (C<ferm: apply ferm.conf (ip ip6, nft):
+12/-3 rules, 1 policy>), whose body details the delta per family,
and whose trailers record the exact program version and command
line (C<Ferm-Version>, C<Ferm-Command>) for forensics.  Disable
with B<--no-etckeeper>.
...
Success.  Under B<--plan>: no pending changes; under a gated
B<--lint>: no findings at or above the gate.

=item B<1>

An error: configuration, kernel tool, usage or rollback failure.

=item B<2>

B<--plan>: changes are pending.  B<--lint> with B<--lint-strict> /
B<--lint-fail-level>: findings at or above the gate.
...
iptables and Python 3.11 or newer (the standalone binary package bundles
its own Python); the native nft backend (B<--nft>) additionally needs
the C<nft> tool
