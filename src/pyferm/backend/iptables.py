...
def extract_table_from_save(save: str, table: str) -> str:
    r"""Return a table's body from a save dump, else ``""`` (Perl ``:3014``).
...
def rules_to_save(
    ...
    *,
    ...
    """
    tool = re.sub(r".*/", "", domain_info.tools["tables-save"])
...
class IptablesBackend(Backend):
    ...
    def rollback(
        ...
        *,
        ...
        if "tables-restore" not in domain_info.tools:
            print(
                f"Cannot rollback domain '{domain}' because there is no "
                f"{domain}tables-restore",
                file=sys.stderr,
            )
            return
